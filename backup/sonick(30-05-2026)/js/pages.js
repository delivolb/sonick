  ...
  try {
    if (db) {
      ...
      recentShips.forEach(s => {
        if (s.status === 'Delivered' || s.status === t('delivered')) stats.delivered++;
        else if (s.status === 'Returned' || s.status === t('statusReturned')) stats.returned++;
        else stats.pending++;
        stats.totalDol += (s.priceDollar || 0);
        stats.totalLeb += (s.priceLeb    || 0);
        if (can('canViewProfit')) stats.profit += (s.deliveryProfit || 0);
      });
    }
  } catch (e) {
...
async function renderShipments() {
  ...
  const showProfit = can('canViewProfit');
  const statusOpts = billtypes_cache.length
    ? billtypes_cache.map(bt => `<option>${esc(bt.name)}</option>`).join('')
...
function shipmentFormHTML(data) {
  ...
  const statusOptions     = billtypes_cache.length
    ? billtypes_cache.map(bt => `<option ${d.status === bt.name ? 'selected' : ''}>${esc(bt.name)}</option>`).join('')
    : ['Pending', 'Delivered', 'Returned', 'Cancelled'].map(s => `<option ${d.status === s ? 'selected' : ''}>${s}</option>`).join('');
  ...
  <div class="form-row">
    ...
    <div class="form-group">
      ...
      <select id="f-status" class="form-select"><option value="">— Select Status —</option>${statusOptions}</select>
    </div>
  </div>
  <div class="form-row">
    <div class="form-group">
...
async function renderGeneral() {
  ...
  ships.forEach(s => {
    ...
    if (s.status === 'Delivered' || s.status === t('delivered')) byCompany[ck].delivered++;
  ...
  const totalProfit= ships.reduce((a, s) => a + (s.deliveryProfit || 0), 0);
